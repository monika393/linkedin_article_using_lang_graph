...
def main():
    ...
    try:
        ...
        if 'critique_feedback' in result:
        ...
        revisions = result.get('revisions_made', 0)
        print(f"\nRevisions Made: {revisions}")
        
        # Show agent call hierarchy
        if 'agent_call_log' in result:
        ...
        logger.info(f"Generation completed - Article length: {len(result['article'])} chars, Revisions: {result['revisions_made']}")
